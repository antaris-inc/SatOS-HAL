...
void csp_iflist_print(void) {
	...
	while (i) {
		...
		DEBUG_CPRINT(("%-10s tx: %05"PRIu32" rx: %05"PRIu32" txe: %05"PRIu32" rxe: %05"PRIu32"\r\n"
...
void csp_upd_tm(void) {
    ...
    while(i) {
        ...
        DEBUG_CPRINT(("%-10s tx: %05"PRIu32" rx: %05"PRIu32" txe: %05"PRIu32" rxe: %05"PRIu32"\r\n"
                "           drop: %05"PRIu32" autherr: %05"PRIu32 " frame: %05"PRIu32"\r\n"
                "           txb: %"PRIu32" rxb: %"PRIu32" MTU: %u\r\n\r\n",
                i->name, i->tx, i->rx, i->tx_error, i->rx_error, i->drop,
                i->autherr, i->frame, i->txbytes, i->rxbytes, i->mtu));
        i = i->next;
        ...
        {
            break;
            DEBUG_CPRINT(("Error: CSP Interfaces overflow"));
        }
    }
}
