...
void csp_ping_noreply(uint8_t node) {
    ...
    DEBUG_CPRINT(("Ping ignore reply node %u.\r\n", (unsigned int) node));
...
void csp_memfree(uint8_t node, uint32_t timeout) {
    ...
    if (err == CSP_ERR_NONE) {
        DEBUG_CPRINT(("Free Memory at node %u is %"PRIu32" bytes\r\n", node, memfree));
    } else {
        DEBUG_CPRINT(("Network error\r\n"));
    }
...
void csp_buf_free(uint8_t node, uint32_t timeout) {
    ...
    if (err == CSP_ERR_NONE) {
        DEBUG_CPRINT(("Free buffers at node %u is %"PRIu32"\r\n", node, size));
    } else {
        DEBUG_CPRINT(("Network error\r\n"));
    }
...
void csp_uptime(uint8_t node, uint32_t timeout) {
    ...
    if (err == CSP_ERR_NONE) {
        DEBUG_CPRINT(("Uptime of node %u is %"PRIu32" s\r\n", node, uptime));
    } else {
        DEBUG_CPRINT(("Network error\r\n"));
    }
