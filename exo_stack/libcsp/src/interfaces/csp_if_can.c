...
#ifdef CSP_DEBUG 
#pragma GCC diagnostic ignored "-Wformat"
		DEBUG_CPRINT(("\nReceived CSP header: %X",buf->packet->id.ext));
#pragma GCC diagnostic pop
#endif
	...
	case CFP_MORE:
		...
		DEBUG_CPRINT(("\nCSP Packet received of length: %d\n",buf->packet->length));

		int i =0;
		DEBUG_CPRINT(("\n\rCAN RX Data: "));
		for(i=0;i<buf->packet->length;i++)
		{
			DEBUG_CPRINT(("%d ",buf->packet->data[i]));
		}
		DEBUG_CPRINT(("\n"));
#endif
		/* Data is available */
...
#ifdef CSP_DEBUG
        DEBUG_CPRINT(("\nCSP Packet sent of length: %d\n",packet->length));

        int i =0;
        DEBUG_CPRINT(("\n\rCAN TX Data: "));
        for(i=0;i<packet->length;i++)
        {
            DEBUG_CPRINT(("%d ",packet->data[i]));
        }
        DEBUG_CPRINT(("\n"));
#endif
