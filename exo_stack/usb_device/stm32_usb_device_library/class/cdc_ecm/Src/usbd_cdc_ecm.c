...
//  uint32_t count=0;
  ...
  if (pdev->pClassDataCmsit[pdev->classId] == NULL)
  {
    //DEBUG_CPRINT(("\n\rUSBD_CDC_ECM_TransmitPacket -FAIL "));
    return (uint8_t)USBD_FAIL;
  }
...
#if 0
    //DEBUG_CPRINT(("\n\rUSBD_CDC_ECM_TransmitPacket -%d \n",hcdc->TxLength));
    DEBUG_CPRINT(("\n\rTX ->"));
    data = (hcdc->TxBuffer);
    count= hcdc->TxLength;
