...
static void kiss_driver_rx(void * user_data, uint8_t * data, size_t data_size, void * pxTaskWoken) {
	...
	csp_uhf_rx(&ctx->iface, data, data_size, NULL);
}

int csp_usart_open_and_add_kiss_interface(ioal_uart_hdle *uart_hdl, const char * ifname, csp_iface_t ** return_iface) {
	...
	if (res == CSP_ERR_NONE) {
		res = csp_usart_open(uart_hdl, kiss_driver_rx, ctx, &ctx->fd);
	}
